...
# OzVPS Production Installer
# Installs from GitHub main branch to app.ozvps.com.au
...
APP_PORT="3000"
DOMAIN="app.ozvps.com.au"
GITHUB_BRANCH="main"
GITHUB_REPO="rorywood/ozvps"

echo -e "${CYAN}${BOLD}"
echo "╔════════════════════════════════════════╗"
echo "║   OzVPS Production Installer          ║"
echo "║   Branch: main                        ║"
echo "║   Domain: app.ozvps.com.au            ║"
echo "╚════════════════════════════════════════╝"
echo -e "${NC}"
...
if [ "$EUID" -ne 0 ]; then
    ...
    echo "Usage: curl -sSL https://raw.githubusercontent.com/${GITHUB_REPO}/${GITHUB_BRANCH}/public/install-prod.sh | sudo bash"
    exit 1
fi
...
# Find the extracted directory (GitHub creates ozvps-<branch> directory)
...
read -p "PostgreSQL Connection String [postgresql://ozvps:password@localhost:5432/ozvps]: " DB_URL
DB_URL=${DB_URL:-postgresql://ozvps:password@localhost:5432/ozvps}
...
echo ""
echo -e "${BOLD}Stripe Configuration (LIVE KEYS)${NC}"
...
AUTH0_BASE_URL=https://app.ozvps.com.au
...
cat > /etc/nginx/sites-available/ozvps-prod << NGINXEOF
server {
    listen 80;
    server_name ${DOMAIN};
...
# Enable site
ln -sf /etc/nginx/sites-available/ozvps-prod /etc/nginx/sites-enabled/
...
curl -fsSL "https://raw.githubusercontent.com/${GITHUB_REPO}/${GITHUB_BRANCH}/public/update-prod.sh" \
    -o /usr/local/bin/update-ozvps-prod
chmod +x /usr/local/bin/update-ozvps-prod
...
echo -e "2. Add your API keys (VirtFusion, Stripe, Auth0)"
echo -e "3. Restart application: ${BOLD}pm2 restart ${SERVICE_NAME}${NC}"
echo ""
echo -e "${BOLD}Useful Commands:${NC}"
echo -e "  Update:  ${BOLD}sudo update-ozvps-prod${NC}"
