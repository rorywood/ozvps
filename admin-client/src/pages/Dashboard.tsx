    ...
    <div>
            ...
            {health?.services.map((service) => (
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  {service.message && (
                    <span className="text-sm text-gray-500">({service.message})</span>
                  )}
                </div>
                <div className="flex items-center gap-3">
      ...
      {health?.system && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-white rounded-xl shadow-sm p-6">
            <h3 className="text-sm font-medium text-gray-500 mb-3">Memory Usage</h3>
            <div className="space-y-2">
              <div className="flex justify-between text-sm">
                <span className="text-gray-600">Used</span>
                <span className="font-medium">{health.system.memory.used} MB</span>
              </div>
          ...
          <div className="bg-white rounded-xl shadow-sm p-6">
            <h3 className="text-sm font-medium text-gray-500 mb-3">CPU Load</h3>
            <div className="space-y-2">
              <div className="flex justify-between text-sm">
                <span className="text-gray-600">1 min avg</span>
                <span className="font-medium">{health.system.cpu.loadAvg["1min"]}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-gray-600">5 min avg</span>
                <span className="font-medium">{health.system.cpu.loadAvg["5min"]}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-gray-600">15 min avg</span>
                <span className="font-medium">{health.system.cpu.loadAvg["15min"]}</span>
              </div>
              <p className="text-xs text-gray-500 mt-2">{health.system.cpu.cores} cores</p>
            </div>
          </div>

          <div className="bg-white rounded-xl shadow-sm p-6">
            <h3 className="text-sm font-medium text-gray-500 mb-3">System Info</h3>
