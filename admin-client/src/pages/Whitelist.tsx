...
export default function Whitelist() {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-gray-900 mb-6">IP Whitelist</h1>
      ...
      {bootstrapMode && (
        <div className="mb-6 p-4 bg-yellow-50 border border-yellow-200 rounded-xl flex items-start gap-3">
          ...
          <div>
            <h3 className="font-semibold text-yellow-800">Bootstrap Mode Active</h3>
      ...
      <div className="bg-white rounded-xl shadow-sm p-6 mb-6">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-lg font-semibold text-gray-900">Your Current IP</h2>
            <p className="text-2xl font-mono text-gray-700 mt-1">{currentIp?.ip || "Loading..."}</p>
      ...
      <div className="bg-white rounded-xl shadow-sm p-6 mb-6">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-semibold text-gray-900">Whitelist Entries</h2>
          <button
            ...
            <table className="w-full">
              ...
              <tbody className="divide-y divide-gray-200">
                {entries?.entries?.map((entry: any) => (
                  <tr key={entry.id} className={!entry.enabled ? "bg-gray-50 opacity-60" : ""}>
                    <td className="px-4 py-3 font-mono">
                      {entry.ipAddress}
                      {entry.cidr && <span className="text-gray-400">{entry.cidr}</span>}
                      {entry.ipAddress === currentIp?.ip && (
                        <span className="ml-2 px-2 py-0.5 bg-blue-100 text-blue-700 text-xs rounded-full">You</span>
                      )}
                    </td>
                    <td className="px-4 py-3">{entry.label}</td>
                    <td className="px-4 py-3 text-sm text-gray-500">{entry.addedByEmail}</td>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <button
                          ...
                          className={`p-1 rounded ${entry.enabled ? "text-yellow-600 hover:bg-yellow-50" : "text-green-600 hover:bg-green-50"}`}
