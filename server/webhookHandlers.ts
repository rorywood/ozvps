    ...
    if (event.type === 'checkout.session.completed') {
      ...
      if (session.metadata?.type === 'wallet_topup') {
        ...
        try {
          ...
          // Note: Invoice is auto-generated by Stripe via invoice_creation on checkout session

          // Try to reactivate any unpaid/suspended servers after wallet top-up
          try {
            await retryUnpaidServers(auth0UserId);
            log(`Retried billing for unpaid/suspended servers for user=${auth0UserId}`, 'stripe');
          } catch (retryError: any) {
            log(`Failed to retry unpaid servers: ${retryError.message}`, 'stripe');
            // Don't throw - we've already credited the wallet successfully
          }
        } catch (error: any) {
