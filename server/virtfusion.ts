...
export class VirtFusionClient {
  ...
  async enableRescueMode(serverId: string): Promise<{ success: boolean; taskId?: string; credentials?: { username: string; password: string } }> {
    try {
      log(`Enabling rescue mode for server ${serverId}`, 'virtfusion');
      
      // VirtFusion rescue mode endpoint - enables rescue boot environment
      const data = await this.request<{ data: any }>(`/servers/${serverId}/rescue/enable`, {
        method: 'POST',
      });
      
      // VirtFusion may return credentials in the response
      const credentials = data.data?.credentials ? {
        username: data.data.credentials.username || 'root',
      ...
      log(`Failed to enable rescue mode for server ${serverId}: ${error}`, 'virtfusion');
      throw error;
    }
  }

  async disableRescueMode(serverId: string): Promise<{ success: boolean; taskId?: string }> {
    try {
      log(`Disabling rescue mode for server ${serverId}`, 'virtfusion');
      
      // VirtFusion rescue mode endpoint - disables rescue and reboots to normal
      const data = await this.request<{ data: any }>(`/servers/${serverId}/rescue/disable`, {
        method: 'POST',
      });
      ...
      log(`Failed to disable rescue mode for server ${serverId}: ${error}`, 'virtfusion');
      throw error;
    }
  }

  async getRescueStatus(serverId: string): Promise<{ 
    isActive: boolean; 
    isEnabling: boolean;
    isDisabling: boolean;
    taskProgress?: number;
    ...
    try {
      // First check the server state for rescue mode status
      const serverData = await this.request<{ data: any }>(`/servers/${serverId}?with[]=remoteState`);
      const server = serverData.data;
      
      // Check if server is in rescue mode - VirtFusion may have a rescue flag
      const isRescueActive = server.rescueMode === true || 
                             server.rescue === true || 
                             server.inRescue === true ||
                             server.settings?.rescueMode === true;
      
      // Check for any pending rescue tasks
      ...
      try {
        const taskData = await this.request<{ data: any }>(`/servers/${serverId}/rescue/status`);
        if (taskData.data) {
          isEnabling = taskData.data.action === 'enable' && taskData.data.status === 'pending';
          isDisabling = taskData.data.action === 'disable' && taskData.data.status === 'pending';
          taskProgress = taskData.data.progress;
        }
      } catch (e) {
        // Status endpoint may not exist or return 404 when no task is running
      }
      ...
      log(`Failed to get rescue status for server ${serverId}: ${error}`, 'virtfusion');
      return {
        isActive: false,
        isEnabling: false,
        isDisabling: false,
  ...
  async isRescueModeSupported(serverId: string): Promise<boolean> {
    try {
      // Check if server supports rescue mode (x86 architecture check)
      const serverData = await this.request<{ data: any }>(`/servers/${serverId}`);
      const server = serverData.data;
      ...
      const rescueDisabled = server.rescueDisabled === true || 
                             server.settings?.rescueDisabled === true;
      
      return isX86 && !rescueDisabled;
    } catch (error) {
      log(`Failed to check rescue support for server ${serverId}: ${error}`, 'virtfusion');
      return false;
    }
  }
